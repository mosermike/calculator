 ...
 * 
 * @file standards.hpp 
 * @note Contains different standard functions
 * 
*/

#ifndef __STANDARDS_HPP_CALC__
#define __STANDARDS_HPP_CALC__
...
// 			value =  stream.str();}
		...
		*/
		...
		string operator+(const std::string add);				// Zwei Strings addieren
		string operator+(string add);
		string operator+=(const char* add);
		string operator+=(const std::string add);				// Zwei Strings addieren
		string operator+=(string add);	// Zwei Strings addieren
		...
		*/
		...
		std::size_t find(string name){return value.find(name.str());}
		std::size_t find(string name,std::size_t old){return value.find(name.str(),old);}
		 ...
		 * @author Mike
		 */
		std::size_t find_back(string name, std::size_t pos = 0, std::size_t low = 0);
		...
		* @author Mike
		*/
		std::vector<std::string> ls(math::shell::arg args, std::string name_include, std::string name_exclude, std::vector<std::string> &directories);
		std::vector<std::string> ls(math::shell::arg args, std::string name_include = "", std::string name_exclude = "");
		...
		*/
		...
		std::string replace(string str_old, string str_new, bool save = false, std::size_t pos = 0, bool all = true);
	...
	private:
		std::string replace_intern(string str_old, string str_new, std::size_t pos, bool save = false);
	...
	public:
		...
		*/
		 ...
		 * @author Mike
		*/
		string substr(std::size_t pos1, std::size_t length1);
		...
		*/
		 ...
		 * @author Mike
		 */
		string sub(std::size_t beg, int32_t end);
	...
	*/
	 ...
	 * @author Mike
	*/
	std::string date(string value = "Datum", std::string trennzeichen1 = ":", std::string trennzeichen2 = ".");
	...
	/**
	 * @note Wert in math::string umwandeln
	 * 
