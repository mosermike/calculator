...
#ifndef __SHELL_HPP_CALC__
#define __SHELL_HPP_CALC__ 
#include <iostream>
#include <vector>
#include <string>
...
namespace math{
	...
	namespace shell
	{
		class arg : public math::vector<math::string> {
		protected:
		
			int argc = 0;
		
			char **argv = {};
 			//std::vector<math::string> vec = {};
		
		public:
		
			// Mit Initialisierung eins Vektors:
// 			arg(std::vector<math::string> vec_temp) :  vec(vec_temp){}
			...
			arg(int argc1, char **argv1, bool getarg = false, std::string except1 = "xxxxx", std::string except2 = "xxxxx", std::string except3 = "xxxxx", std::string except4 = "xxxxx", std::string except5 = "xxxxx", std::string except6 = "xxxxx", std::string except7 = "xxxxx") {
				...
				std::vector<string> vec_temp(argv,argv+argc);
				
				vec = vec_temp;
				
				if(getarg)
					getArg(except1, except2, except3, except4,except5,except6,except7);
			}	// Aufruf math::shell::arg arg(argc,argv);
			...
			arg(char **argv1, int argc1) {
				...
				std::vector<math::string> vec_temp(argv,argv+argc);
				
				vec = vec_temp;
			}	// Aufruf math::shell::arg arg(argv,argc);
			
			// Mit direkter Wertübergabe:
			arg(math::string a){
				vec.push_back(a);
			}
			arg(math::string a, math::string b){
				vec.push_back(a);
				vec.push_back(b);
			}
			arg(math::string a, math::string b, math::string c){
				vec.push_back(a);
				vec.push_back(b);
				vec.push_back(c);
			}
			 ...
			 * @author Mike
			 */
			std::vector<math::string> &operator()(char **argv1, int argc1, bool getarg = false);		// bestimmte Position ausgeben
			std::vector<math::string> &operator()(int argc1, char **argv1, bool getarg = false);
			...
			*/
			 ...
			 * @author Mike
			 */
			std::vector<math::string> replace(char sign_old, char sign_new);
			std::vector<math::string> replace(std::string str_old, std::string str_new);
			 ...
			 * @author Mike
			 */
			std::vector<math::string> str();	// Umwandlung zu string
			std::vector<std::string> str_change();	// Umwandlung zu std::string
			std::vector<math::string> tostr(std::vector<std::string> temp); 	// Umwandlung zu math::string
		...
		*/
		 ...
		 * @author Mike
		 */
		void cout(math::string text, bool newline = true, std::string colour = "lightgreen");
		/**
